             ...
             atflv       TYPE atwrt,
             counted_pc  TYPE iseg-menge,
             counted_qty TYPE iseg-menge,
             meins       TYPE iseg-meins,
             zero_count  TYPE xfeld,
          ...
          WHEN 5.
            CALL FUNCTION 'CONVERSION_EXIT_MATN1_INPUT'
              ...
              EXCEPTIONS
                length_error = 1
                OTHERS       = 2.
          WHEN 6. ls_list-maktx = value.
          WHEN 7. ls_list-atwrt = value.
          WHEN 8. ls_list-kmlmt = value.
          WHEN 9. ls_list-kunnr = |{ value ALPHA = IN }|.
          WHEN 10. ls_list-name1 = value.
          WHEN 11. ls_list-kdauf = value.
          WHEN 12. ls_list-kdpos = value.
          WHEN 13. ls_list-lgort = value.
          WHEN 14. ls_list-lgobe = value.
          WHEN 15. ls_list-btext = value.
          WHEN 16. ls_list-charg = value.
          WHEN 17. ls_list-atflv = value.
          WHEN 18. ls_list-counted_pc = value.
          WHEN 19. ls_list-counted_qty = value.
          WHEN 20. ls_list-meins = value.
          WHEN 21. ls_list-zero_count = value.
        ENDCASE.
        column = column + 1.
      ENDWHILE.
...
*    ENDIF.
  ...
  METHOD save.
    ...
    DATA(lt_doc) = t_list.
    DELETE lt_doc WHERE counted_qty IS INITIAL
                  AND   zero_count  IS INITIAL.
    SORT lt_doc BY iblnr gjahr.
    DELETE ADJACENT DUPLICATES FROM lt_doc COMPARING iblnr gjahr.
    ...
    LOOP AT lt_doc ASSIGNING FIELD-SYMBOL(<ls_doc>).
      ...
      LOOP AT t_list ASSIGNING FIELD-SYMBOL(<ls_list>) WHERE iblnr  = <ls_doc>-iblnr
                                                       AND ( counted_qty <> 0
                                                       OR    zero_count = abap_true ).
        SELECT SINGLE werks FROM iseg
          WHERE iblnr = @<ls_doc>-iblnr
            AND gjahr = @<ls_doc>-gjahr
            AND zeili = @<ls_list>-zeili
          INTO @DATA(lv_werks).
        ...
        IF sy-subrc <> 0.
          MESSAGE ID 'ZMM' TYPE 'E' NUMBER '015' WITH lv_werks '' '' '' INTO <ls_list>-message.
          <ls_list>-statu = '1'.
          CLEAR lv_werks.
          CONTINUE.
        ENDIF.
        ...
        <ls_item>-entry_qnt = <ls_list>-counted_qty.
        <ls_item>-entry_uom = <ls_list>-meins.
        <ls_item>-zero_count = <ls_list>-zero_count.

      ENDLOOP.

      CALL FUNCTION 'BAPI_MATPHYSINV_COUNT'
        EXPORTING
          physinventory = CONV ikpf-iblnr( |{ <ls_doc>-iblnr ALPHA = IN }| )
